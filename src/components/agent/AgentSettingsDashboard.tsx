  ...
  return <div className="min-h-screen bg-[var(--background)]">
    ...
    <main className={`mt-16 min-h-[calc(100vh-64px)] p-[18px] transition-[margin] 2xl:p-6 ${collapsed ? "ml-[72px]" : "ml-[220px]"}`}><div className="mx-auto min-w-[1000px] max-w-[1500px] space-y-4">
      ...
      {!form || agent.isPending ? <div className="panel-card p-8 text-center text-xs text-[var(--text-muted)]">正在加载智能体配置…</div> : <form onSubmit={submit} className="panel-card grid grid-cols-2 gap-5 p-6">
        ...
        <NumberField label="销冠检索数量" value={form.champion_top_k} min={1} max={10} step={1} onChange={(value) => set("champion_top_k", value)} disabled={!canEdit} />
        <NumberField label="销冠最低相关度" value={form.champion_min_score} min={0} max={1} step={0.05} onChange={(value) => set("champion_min_score", value)} disabled={!canEdit} />
        <NumberField label="语义相似度权重" value={form.champion_semantic_weight} min={0} max={1} step={0.05} onChange={(value) => set("champion_semantic_weight", value)} disabled={!canEdit} />
        <NumberField label="行业匹配权重" value={form.champion_industry_weight} min={0} max={1} step={0.05} onChange={(value) => set("champion_industry_weight", value)} disabled={!canEdit} />
        <NumberField label="销售阶段权重" value={form.champion_stage_weight} min={0} max={1} step={0.05} onChange={(value) => set("champion_stage_weight", value)} disabled={!canEdit} />
        <NumberField label="历史成功权重" value={form.champion_success_weight} min={0} max={1} step={0.05} onChange={(value) => set("champion_success_weight", value)} disabled={!canEdit} />
        <NumberField label="管理员评分权重" value={form.champion_admin_score_weight} min={0} max={1} step={0.05} onChange={(value) => set("champion_admin_score_weight", value)} disabled={!canEdit} />
        ...
        <div className="space-y-3 rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] p-4 text-[10px]"><label className="flex items-center gap-2"><input type="checkbox" checked={form.allow_emoji} onChange={(e) => set("allow_emoji", e.target.checked)} disabled={!canEdit} />允许表情</label><label className="flex items-center gap-2"><input type="checkbox" checked={form.require_citations} onChange={(e) => set("require_citations", e.target.checked)} disabled={!canEdit} />强制知识引用</label><label className="flex items-center gap-2"><input type="checkbox" checked={form.champion_enabled} onChange={(e) => set("champion_enabled", e.target.checked)} disabled={!canEdit} />启用销冠知识库</label><label className="flex items-center gap-2"><input type="checkbox" checked={form.champion_prefer_tenant} onChange={(e) => set("champion_prefer_tenant", e.target.checked)} disabled={!canEdit} />优先使用本企业销冠经验</label><label className="flex items-center gap-2"><input type="checkbox" checked={form.champion_allow_general_generation} onChange={(e) => set("champion_allow_general_generation", e.target.checked)} disabled={!canEdit} />无销冠经验时允许通用生成</label><p className="text-[var(--text-muted)]">当前配置版本：v{form.version} · 权重总和应接近 1</p></div>
