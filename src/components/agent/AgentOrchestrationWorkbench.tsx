...
export function AgentOrchestrationWorkbench() {
  ...
  const set = <K extends keyof Draft>(key: K, value: Draft[K]) => {
    setDraft((current) => ({ ...(current || configQuery.data || form), [key]: value } as Draft));
    setIsDirty(true);
  };
  ...
  const buildPayload = () => {
    ...
    if (!isAdmin) {
      delete payload.identity_prompt;
      delete payload.prohibited_claims;
      delete payload.human_handoff_rules;
    }
    return payload;
  };
  const saveDraft = async () => {
    if (!canEdit) return;
    try {
      const saved = await update.mutateAsync(buildPayload());
      setDraft(saved);
      setIsDirty(false);
  ...
  const publishDraft = async () => {
    ...
    try {
      if (isDirty) await update.mutateAsync(buildPayload());
      const value = await publish.mutateAsync();
      setDraft(value);
      setIsDirty(false);
      showToast("配置已发布", "success");
    } catch (error) { showToast(error instanceof Error ? error.message : "发布失败", "error"); }
    finally { publishLock.current = false; }
  };
  const restorePublished = async () => {
    try { const value = await restore.mutateAsync(); setDraft(value); setIsDirty(false); showToast("已恢复最近发布版本", "success"); } catch (error) { showToast(error instanceof Error ? error.message : "恢复失败", "error"); }
  };
  const runTest = async () => {
  ...
  return <div className="min-h-screen bg-[var(--background)] text-[var(--navy)]">
    ...
    <main className={`mt-16 min-h-[calc(100vh-64px)] px-4 py-5 transition-[margin] md:px-6 ${collapsed ? "ml-[72px]" : "ml-[220px]"}`}>
      <div className="mx-auto max-w-[1440px] space-y-5">
        ...
        <div className="space-y-5"><ConfigSection title="一、基础身份" eyebrow="Identity" description="让团队知道智能体是谁、如何与客户沟通。"><div className="grid gap-4 md:grid-cols-2"><Field label="智能体名称"><input value={form.agent_name} onChange={(event) => set("agent_name", event.target.value)} disabled={!canEdit} className="form-input h-11 disabled:bg-slate-50" /></Field><Field label="身份说明"><textarea value={form.identity_prompt} onChange={(event) => set("identity_prompt", event.target.value)} disabled={!canEdit || !isAdmin} className="form-input min-h-24 md:row-span-2" /></Field><Select label="回复风格" value={form.reply_style} options={styleOptions} onChange={(value) => set("reply_style", value)} disabled={!canEdit} /><Select label="回复长度" value={form.reply_length} options={lengthOptions} onChange={(value) => set("reply_length", value)} disabled={!canEdit} /><Select label="销售主动程度" value={form.sales_aggressiveness} options={aggressivenessOptions} onChange={(value) => set("sales_aggressiveness", value)} disabled={!canEdit} /><Toggle label="允许表情" checked={form.allow_emoji} onChange={(value) => set("allow_emoji", value)} disabled={!canEdit} /></div><Field label="自定义说明"><textarea value={form.custom_instructions || ""} onChange={(event) => set("custom_instructions", event.target.value)} disabled={!canEdit} placeholder="例如：面向制造业客户，优先使用 ROI 和实施节奏解释方案。" className="form-input min-h-24" /></Field></ConfigSection>
